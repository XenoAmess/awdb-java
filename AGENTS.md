...
- Release: creating a GitHub Release triggers `.github/workflows/release.yml` → `mvn deploy -Prelease` → Maven Central (Central Portal, `central-publishing-maven-plugin`, autoPublish). The `release` profile adds sources/javadoc jars + GPG signing. Signing key: `CA7447825A093152` ("XenoAmess (awdb-java release signing)", on keyserver.ubuntu.com); its private key + passphrase live in the **actions** secrets `GPG_PRIVATE_KEY` / `GPG_PASSPHRASE`, alongside `CENTRAL_USERNAME` / `CENTRAL_TOKEN` (Central Portal token, NOT the legacy OSSRH one). The `com.xenoamess` namespace is verified in the publisher's Central Portal account.
