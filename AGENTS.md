...
- Release: creating a GitHub Release triggers `.github/workflows/release.yml` → `mvn deploy -Prelease` → Maven Central (Central Portal, `central-publishing-maven-plugin`, autoPublish). The `release` profile adds sources/javadoc jars + GPG signing. Required **actions** secrets: `GPG_PRIVATE_KEY`, `GPG_PASSPHRASE`, `CENTRAL_USERNAME`, `CENTRAL_TOKEN` (Central Portal token, NOT the legacy OSSRH one). The `com.xenoamess` namespace must be verified in the publisher's Central Portal account beforehand.
